#include "blFinderAdminCategoryEdit.h"

blFinderAdminCategoryEdit::blFinderAdminCategoryEdit(QWidget *parent)
    : QWidget(parent){
    ...
    // title
    QLabel *title = new QLabel(tr("Edit category"), this);
...
void blFinderAdminCategoryEdit::setInfo(blFinderCategoryInfo info){
    m_id = info.id();
    m_name->setText(info.name());
    m_removeButton->setVisible(true);
}

void blFinderAdminCategoryEdit::emitEditInfo(){
...
void blFinderAdminCategoryEdit::emitRemove(){
    int reponse = QMessageBox::question(this, tr("Delete category"), tr("Delete this category ?"), QMessageBox::Yes | QMessageBox::No);
